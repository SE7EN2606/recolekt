...
export const EditableBullets: React.FC<EditableBulletsProps> = ({ 
  ...
  const addBullet = () => {
    onChange([...value, { headline: '', text: '', emoji: '•' }]);
  };
  ...
  if (!bullets.length && !isEditMode) return null;
    ...
    <ul className={mobile ? 'space-y-2' : 'space-y-3'}>
      {bullets.map((b: any, i: number) => (
        <li key={i} className={`flex ${mobile ? 'gap-2' : 'gap-3'} items-start`}>
          <span className={mobile ? 'text-base' : 'text-lg min-w-[24px]'}>{b.emoji || '•'}</span>
          <div className="flex-1">
            <p className={`text-${mobile ? 'xs' : 'sm'} font-semibold text-gray-900 ${mobile ? 'mb-0.5' : 'mb-1'}`}>
              {b.headline}
            </p>
            <p className={`text-${mobile ? 'xs' : 'sm'} ${mobile ? 'text-gray-600' : 'text-gray-700'}`}>
              {b.text}
            </p>
          </div>
        </li>
...
export const EditableHashtags: React.FC<EditableHashtagsProps> = ({ 
  ...
  if (!hashtags.length && !isEditMode) return null;
