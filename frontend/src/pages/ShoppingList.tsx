          ...
          <main className="space-y-6">
            ...
            {groupedItems.length === 0 ? (
              <div className="rounded-[28px] border border-dashed border-gray-200 bg-white/60 p-8 text-center">
                ...
                <p className="font-black text-gray-900">Your derived grocery list will appear here</p>
                <p className="mt-1 text-sm font-medium text-gray-500">Add ingredients from a recipe to get started.</p>
              </div>
            ) : (
                ...
                <section key={group.title} className="space-y-3">
                  ...
                  <div className="overflow-hidden rounded-[24px] border border-gray-100 bg-white shadow-sm">
                    {group.items.map((item) => (
                      <div key={item.key} className="border-b border-gray-100 p-4 last:border-b-0">
                        <div className="flex items-start gap-3">
                          <button
                            type="button"
                            onClick={() => patchItem(item.key, { checked: !item.checked })}
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-1">
                              {formatQuantity(item) && (
                                <span className="text-sm font-black text-emerald-700">{formatQuantity(item)}</span>
                              )}
                              <span className={`text-sm font-black ${item.checked ? 'text-gray-400 line-through' : 'text-gray-950'}`}>
                                {item.name}
                              </span>
                            </div>
                          ...
                          <button
                            type="button"
                            onClick={() => patchItem(item.key, { excluded: true })}
                            className="shrink-0 rounded-xl px-2.5 py-1.5 text-[11px] font-black text-gray-400 transition-colors hover:bg-amber-50 hover:text-amber-700"
                          >
                            I have this
                          </button>
                        </div>
