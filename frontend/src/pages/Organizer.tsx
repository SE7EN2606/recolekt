...
export const Organizer: React.FC = () => {
  ...
  // ✅ UPDATED ICONS: Size 20 and text-primary-600 for the main options
  const mainSelectorOptions: DropdownOption[] = [
    { value: 'all', label: 'All Videos', icon: <LayoutGrid size={20} className="text-primary-600" /> },
    { value: 'unsorted', label: 'Unsorted', icon: <Inbox size={20} className="text-primary-600" /> },
    { value: 'favorites', label: 'Favorites', icon: <Heart size={20} className="text-primary-600" /> },
    { divider: true, value: 'div1', label: '' },
    ...folders.flatMap(f => [
      { value: f.id, label: f.name, icon: <Folders size={20} className="text-primary-600" /> },
      ...(f.subFolders || []).map(sub => ({
         value: sub.id, label: sub.name, icon: <FolderOpen size={20} className="text-gray-400 group-hover:text-primary-500" />, isSub: true
      }))
    ])
  ];
  ...
  const parentCreationOptions: DropdownOption[] = [
    { value: '', label: 'Main Level (No Parent)', icon: <LayoutGrid size={16}/> },
    { divider: true, value: 'div1', label: '' },
    ...folders.filter(f => !['all', 'favorites', 'archive', 'unsorted'].includes(f.id) && !f.isSystem).map(f => (
      { value: f.id, label: `Inside "${f.name}"`, icon: <Folders size={16}/> }
    ))
  ];

  return (
    <div className="flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm mt-6 mb-24 md:mb-8 relative z-0">
      ...
      <div className="h-16 border-b border-gray-100 flex items-center justify-between px-4 md:px-6 bg-white rounded-t-2xl z-50 sticky top-0 shadow-sm">
          ...
          {isRenaming ? (
            ...
            <div className="flex items-center gap-2 min-w-0">
              <CustomDropdown 
                ...
                triggerClassName="appearance-none bg-transparent font-black text-xl text-gray-900 outline-none cursor-pointer truncate flex items-center hover:opacity-70 transition-opacity"
              />

              {/* ✅ TEXT MOVED TO THE RIGHT SIDE */}
              <span className="text-gray-400 font-bold text-sm hidden sm:inline-block whitespace-nowrap ml-2">
                Select a folder
              </span>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {/* ✅ CANCEL SELECTION BUTTON (Red, appears only when selecting) */}
          ...
          {lastAction && (
            ...
            >
              <Undo2 size={14} />
              <span className="hidden md:inline">Undo Move</span>
            </button>
          )}
      ...
      <div className="px-4 md:px-6 py-3 bg-gray-50/50 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 z-40 sticky top-16 backdrop-blur-sm">
         ...
         <div className="flex items-center justify-between sm:justify-end gap-2 w-full sm:w-auto overflow-visible">
           ...
           {/* ✅ NEW FOLDER BUTTON HIDDEN WHEN SELECTING */}
           {selectedVideoIds.size === 0 && (
             <button 
               type="button"
           ...
           {selectedVideoIds.size > 0 && (
             <div className="flex items-center gap-3 animate-fade-in bg-primary-50 pl-3 pr-1 py-1 rounded-xl border border-primary-100 ml-auto">
               <span className="text-xs font-bold text-primary-700 whitespace-nowrap">{selectedVideoIds.size} selected</span>
               <CustomDropdown 
                 value=""
      ...
      {isCreating && (
        <div className="relative z-50 px-4 md:px-6 py-4 bg-primary-50/80 border-b border-primary-100 flex flex-col gap-3 animate-fade-in">
          <div className="text-xs font-bold text-primary-700 uppercase tracking-wider flex items-center gap-2">
            <CornerDownRight size={14} /> Create Folder
          </div>
