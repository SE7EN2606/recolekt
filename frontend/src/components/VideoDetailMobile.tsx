...
import { ActionSheet, ActionItem } from './ActionSheet'; // ✅ Use new ActionSheet
...
export const VideoDetailMobile: React.FC<VideoDetailMobileProps> = ({
  ...
  // ✅ Scroll Lock for center modals
  useEffect(() => {
    if (isMoveCollectionOpen || isManageOpen || isNewCollectionOpen) {
  ...
  }, [isMoveCollectionOpen, isManageOpen, isNewCollectionOpen]);
  ...
  // ✅ Maps actions into the new ActionSheet component
  const reelActions: ActionItem[] = useMemo(() => [
    { icon: ExternalLink, label: t('videoDetail:share', 'Share Video'), onClick: onShare },
    { icon: Heart, label: t('videoDetail:addToFavorites', 'Add to Favorites'), onClick: onToggleFavorite ?? (() => {}), variant: 'primary' },
    { icon: FolderClosed, label: t('videoDetail:moveToCollection', 'Move to Collection'), onClick: () => { setIsReelMenuOpen(false); setIsMoveCollectionOpen(true); } },
    { icon: Archive, label: t('videoDetail:archive', 'Archive'), onClick: handleArchive },
    { icon: FolderOpen, label: t('videoDetail:manageCollections', 'Manage Collections'), onClick: () => { setIsReelMenuOpen(false); setIsManageOpen(true); } },
    { icon: CircleAlert, label: t('videoDetail:reportIssue', 'Report Issue'), onClick: () => { setIsReelMenuOpen(false); onReportClick(); } },
    { icon: Trash2, label: t('common:delete', 'Delete Video'), onClick: () => { setIsReelMenuOpen(false); onDeleteClick(); }, variant: 'danger' }
  ], [onShare, onToggleFavorite, handleArchive, onReportClick, onDeleteClick, t]);
  ...
  const imageUrl = viewModel.thumbnailUrl || viewModel.preview || viewModel.gcs_urls?.preview_thumbnail || '';
  ...
  return (
    <div className="md:hidden -mx-4 sm:mx-0">
      ...
      >
        {/* ✅ Image Render Check to prevent empty src error */}
        {imageUrl ? (
          <img
            src={imageUrl}
            alt={displayTitle}
          ...
          <div className="w-full h-full bg-gray-900 opacity-90" />
        )}
      ...
      <div className="px-4 pt-5 pb-16">
        ...
        <div className="flex items-center justify-between mb-4">
          {authorName ? (
            <a
              href={viewModel.sourceUrl || viewModel.originalUrl}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 group/author"
            >
              {viewModel.platform === 'fb' ? (
                <FacebookExternalIcon size={16} className="text-blue-600 flex-shrink-0" />
              ) : (
        ...
        <div className="mb-5 bg-violet-50 border border-violet-200 rounded-xl overflow-hidden p-4">
              ...
              <div className="flex flex-col gap-2 pr-10 flex-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    ...
                    style={{ color: '#e11d48' }}
                  >
                    {viewModel.subCategory || viewModel.topic}
                  </span>
                </div>
              </div>
          ...
          {((viewModel.hashtags || viewModel.tags || []) as string[]).length > 0 && (
            <div className="mt-1">
              ...
              <div className="hashtag-links flex flex-wrap gap-2">
                <EditableHashtags
                  hashtags={viewModel.hashtags || viewModel.tags || []}
                  isEditMode={isEditMode}
                  value={tempHashtags}
        ...
        {!isEditMode && (viewModel.sourceUrl || viewModel.originalUrl) && (
          <a
            href={viewModel.sourceUrl || viewModel.originalUrl}
            target="_blank"
            rel="noreferrer"
            className={`flex items-center justify-center gap-2 px-4 py-3 text-white rounded-xl text-sm font-bold shadow-md transition ${viewModel.platform === 'fb' ? 'bg-[#1877F2] hover:bg-[#166FE5]' : 'bg-gradient-to-r from-[#F58529] via-[#DD2A7B] to-[#8134AF] hover:opacity-90'}`}
          >
            {viewModel.platform === 'fb' ? <FacebookExternalIcon size={16} className="text-white" /> : <ExternalLink size={16} className="text-white" />}
            {viewModel.platform === 'fb' ? 'View on Facebook' : t('videoDetail:viewOnInstagram')}
          </a>
        )}
      </div>

      {/* ✅ ActionSheet replacing MobileReelActions */}
      <ActionSheet 
        isOpen={isReelMenuOpen} 
        onClose={() => setIsReelMenuOpen(false)} 
        actions={reelActions} 
        title={t('videoDetail:manageReel', 'Manage Reel')} 
      />
