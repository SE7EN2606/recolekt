      ...
      `}>
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-900">
              {count > 1
                ...
                : t('modals:moveSingleVideo', 'Move 1 video')}
            </h3>
            <p className="text-gray-500 text-xs mt-0.5">{t('modals:selectDestination', 'Select a new collection')}</p>
          </div>
        ...
        <div className="p-5">
          <div className="relative" ref={dropdownRef}>
            ...
            >
              <FolderOpen size={18} className="text-primary-600 shrink-0 min-w-[18px]" aria-hidden="true" />
            ...
            {isDropdownOpen && (
              <div className="absolute z-50 w-full mt-2 bg-white border border-gray-200 rounded-2xl shadow-xl max-h-60 overflow-y-auto py-2 animate-in fade-in zoom-in duration-200">
                {flatFolders.map((f) => (
                  ...
                  >
                    {f.level > 0 && <CornerDownRight size={16} className="ml-2 text-gray-400 shrink-0" aria-hidden="true" />}
                    <span className={`text-sm flex-1 truncate ${f.level === 0 ? 'font-bold text-gray-900' : 'font-medium text-gray-600'}`}>
                      {f.name}
                    </span>
                    {targetFolderId === f.id && <Check size={16} className="text-primary-600 shrink-0" aria-hidden="true" />}
                  </button>
                ))}
              </div>
            )}
