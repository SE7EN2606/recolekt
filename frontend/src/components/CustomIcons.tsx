...
export const PlatformIconBtn = ({ platform, size = 16, className }: { platform: string; size?: number; className?: string }) => {
  if (platform === 'facebook' || platform === 'fb') {
    return (
      <svg width={size} height={size} fill="currentColor" viewBox="0 0 24 24" className={className ?? "text-white shrink-0"}>
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
      </svg>
    );
  }
  ...
  return (
    <a href={url} target="_blank" rel="noreferrer" className="block w-full">
      <button className={`w-full flex items-center justify-center gap-2 px-5 py-3 rounded-xl text-white font-bold text-sm shadow-sm transition ${c.bg} ${c.hover}`}>
        <PlatformIconBtn platform={platform} size={16} className="text-white shrink-0" />
        {c.label}
      </button>
    </a>
  );
