...
export const ProfileMenu: React.FC<ProfileMenuProps> = ({ isOpen, onClose }) => {
  ...
  const { user, logout } = useData(); // Get user state
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50/50">
          <div className="p-6 pb-40 space-y-8">
            ...
            {/* DYNAMIC CARD: Show "Join" if guest, "User Info" if logged in */}
            {!user ? (
              // GUEST VIEW (Your existing design)
              ...
              // LOGGED IN VIEW (New design matching the style)
              <div className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-6">
                 ...
                 <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 p-4 bg-red-50 text-red-600 rounded-xl font-bold hover:bg-red-100 transition-colors"
                 >
                    <LogOut size={20} />
                    Sign Out
                 </button>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-10">
                ...
                <div className="pl-2">
                  ...
                  <ul className="space-y-3 text-sm text-gray-600 font-medium">
                    <li><Link to="/features" onClick={onClose} className="hover:text-primary-600 transition-colors flex items-center justify-between">Features</Link></li>
                    <li><Link to="/pricing" onClick={onClose} className="hover:text-primary-600 transition-colors flex items-center justify-between">Pricing</Link></li>
                    <li><Link to="/security" onClick={onClose} className="hover:text-primary-600 transition-colors flex items-center justify-between">Security</Link></li>
                  </ul>
                </div>
                ...
                <div className="pl-12">
                  ...
                  <ul className="space-y-3 text-sm text-gray-600 font-medium">
                    <li><Link to="/about" onClick={onClose} className="hover:text-primary-600 transition-colors">About</Link></li>
                    <li><Link to="/blog" onClick={onClose} className="hover:text-primary-600 transition-colors">Blog</Link></li>
                    <li><Link to="/contact" onClick={onClose} className="hover:text-primary-600 transition-colors">Contact</Link></li>
                  </ul>
                </div>
              </div>
            </div>
