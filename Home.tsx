...
export const Home: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        setError(errorData.error || 'This video has already been saved to your collection.');
        setUrl('');
        setIsLoading(false);
        return;
      }
      ...
      if (!response.ok) {
        ...
        setError(errorData.error || 'Failed to import this URL. Please try again.');
        setIsLoading(false);
        return;
      }
  ...
  return (
    <div className="flex flex-col items-center">
      {/* Hero Section - ✅ Removed pt-8 md:pt-16 */}
      <div className="w-full max-w-4xl mx-auto pb-8 md:pb-12 px-4 text-center">
        ...
        <div className="w-full max-w-3xl mx-auto mt-8">
          <div className="flex flex-col gap-4">
            {/* ERROR ALERT with fade-out animation */}
            {error && (
              <div 
            ...
            <div className="relative flex-1">
              ...
              {/* Small RED X button - centered vertically, smaller size */}
              {url && (
                <button 
                  type="button"
              ...
              {/* Desktop Buttons - Paste, Download */}
              <div className="hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 items-center gap-2 z-10">
                ...
                <button 
                  ...
                  className="flex items-center gap-2 px-6 h-[44px] bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-lg transition-all text-sm shadow-lg shadow-primary-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? (
                    <>
            ...
            {/* Download Button - Mobile only (smaller) */}
            <button 
              onClick={handleSave}
              disabled={!url.trim() || isLoading}
              className="md:hidden h-[50px] px-8 bg-primary-600 hover:bg-primary-700 text-white font-bold rounded-xl transition-all text-base shadow-xl shadow-primary-600/20 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <>
        ...
        {/* Checkmarks - Reduced gap below on both mobile and desktop */}
      ...
      {/* Add fadeOut animation to global CSS */}
      <style>{`
        @keyframes fadeOut {
          from { opacity: 1; }
          to { opacity: 0; }
