...
def _save_input_payload(process_id, gcs_paths, temp_dir, gcs_client,
                        caption, transcript, ocr_text, merged_text):
...
def background_process(result, video_path, temp_dir, shortcode, caption, url,
    ...
    try:
        ...
        if is_youtube:
            ...
            _save_input_payload(
                result["process_id"], gcs_paths, temp_dir, gcs_client,
                caption, transcript_text, "", merged_text,
            )
        ...
        _save_input_payload(
            result["process_id"], gcs_paths, temp_dir, gcs_client,
            caption, final_transcript, ocr_text, merged_text,
        )
        ...
        logger.info("🔑 tools_list present: %s | type: %s",
        ...
        _tl = result.get("tools_list")
        if _tl and result.get("process_id"):
            import json as _json
            _tl_str = _json.dumps(_tl, ensure_ascii=False) if isinstance(_tl, dict) else _tl
            try:
                execute(
                    "UPDATE reels SET tools_list = %s::jsonb, content_type = %s WHERE id = %s",
                    (_tl_str, result.get("content_type", "tools"), result.get("process_id")),
                    commit=True,
                )
                logger.info("✅ DIRECT tools_list SAVED for %s", result.get("process_id"))
            except Exception as _tl_err:
                logger.error("❌ DIRECT tools_list FAILED: %s", _tl_err)
