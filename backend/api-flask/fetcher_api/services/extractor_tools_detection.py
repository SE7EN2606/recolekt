...
_PRODUCT_TEST_CATEGORY_HINTS = frozenset({
    "tested above 50", "failed", "best value",
    "s tier", "a tier", "b tier", "c tier", "d tier", "f tier",
})
...
def analyze_structure(
    ...
    if pre_hint in ("picks", "verdict") and rank_text_hits == 0:
        ...
        ratings = {
            (item.get("creator_rating") or "").strip().lower()
        ...
        if is_flat_picks:
            ...
            if verdict_score >= 3:
                return {
                    ...
                    "render_hint": "verdict_board",
                    "list_subtype": "verdict",
                    "is_ranked": False,
                    "confidence": 0.75,
                    "global_ordered": False,
                    "group_ordered": False,
                    "reason": "Pre-hint picks/verdict with verdict labels — no ranking signals",
                }
            return {
                "mode": "structured",
                "structure_type": "grouped",
                "render_hint": "grouped_list",
                "list_subtype": "picks",
                "is_ranked": False,
                "confidence": 0.78,
                "global_ordered": False,
                "group_ordered": False,
                "reason": "Pre-hint picks, no ranking signals, uniform creator_rating — flat picks list",
            }
    ...
    if _single_global_ranking(active):
        ...
        return {
            ...
            "global_ordered": True,
            "group_ordered": True,
            "reason": "_single_global_ranking matched",
        }
    ...
    if verdict_score >= 3:
        return {
            ...
            "list_subtype": "verdict",
            "is_ranked": False,
            "confidence": min(0.70 + (0.05 * min(verdict_score, 5)), 0.95),
            "global_ordered": False,
            "group_ordered": False,
            "reason": f"verdict_label_score={verdict_score}",
        }

    if ranking_label_score >= 2 or rank_text_hits >= 2 or (
        rank_info["has_sequence"] and rank_info["coverage_ratio"] >= 0.75
    ) or _categories_follow_rank_order(active):
        confidence = min(0.65 + (0.05 * min(rank_text_hits + ranking_label_score, 5)), 0.92)
        return {
            "mode": "structured",
            "structure_type": "ranking",
            "render_hint": "ranked_list",
            ...
            "is_ranked": True,
            "confidence": confidence,
            "global_ordered": rank_info["has_sequence"],
            "group_ordered": _categories_follow_rank_order(active),
            "reason": f"rank_text_hits={rank_text_hits} label_score={ranking_label_score}",
        }
    ...
    return {
        ...
        "structure_type": "grouped",
        "render_hint": "grouped_list",
        "list_subtype": "picks" if pre_hint in ("picks", "") else pre_hint,
        "is_ranked": False,
        "confidence": 0.60,
        "global_ordered": False,
        "group_ordered": grouped_score >= 1,
        "reason": "No strong ranking/verdict/tier signals — defaulting to grouped/picks",
    }
