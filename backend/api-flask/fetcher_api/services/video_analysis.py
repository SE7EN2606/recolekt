...
def _crop_portrait_center(image_path: str) -> bool:
    try:
        ...
        if w <= h * 1.2:
            logger.info(f"🖼️ Thumbnail {w}x{h} — already portrait/square, no crop needed")
            return True
        ...
        logger.info(f"✂️ Cropped landscape thumbnail {w}x{h} → {target_w}x{h} (9:16 portrait center)")
        return True
    except Exception as e:
        logger.warning(f"⚠️ Could not crop thumbnail: {e}")
        return False
...
def _download_thumbnail_from_url(
    ...
    try:
        ...
        if size < min_size:
            logger.warning(f"⚠️ Thumbnail too small ({size} bytes), rejecting → {output_path}")
            return False
...
def _download_youtube_thumbnail(video_id: str, output_path: str) -> bool:
    """
    Fetches YouTube thumbnail directly from CDN — no yt-dlp, no FFmpeg.

    Priority:
      1. oardefault.jpg  — native portrait (Shorts 1080×1920), confirmed working
      2. oar2.jpg        — same native portrait, alternate key
      3. maxresdefault   — landscape 1280×720, center-crop to 9:16
    ...
    for url in portrait_candidates:
        if _download_thumbnail_from_url(url, output_path, min_size=5_000, crop_portrait=False):
            logger.info(f"✅ YouTube portrait thumbnail from CDN: {url}")
            return True
    ...
    for url in landscape_candidates:
        if _download_thumbnail_from_url(url, output_path, min_size=20_000, crop_portrait=True):
            logger.info(f"✅ YouTube landscape thumbnail (cropped): {url}")
            return True

    logger.warning(f"⚠️ All YouTube CDN thumbnail URLs failed for video_id={video_id}")
    return False
...
# ── INSTAGRAM THUMBNAIL (legacy instaloader post object) ─────────────────────
...
def fetch_youtube_data(url: str, temp_dir: Optional[str] = None) -> Dict:
    ...
    For YouTube:
      ...
      2. Thumbnail — direct CDN hit (oardefault → oar2 → landscape + crop). No yt-dlp. No FFmpeg.
      3. Transcript via youtube-transcript-api
      4. If no transcript → download audio only for Deepgram fallback
    """
    try:
        ...
        # ── 2. Thumbnail — direct CDN, no yt-dlp, no FFmpeg ─────────
        thumb_out      = os.path.join(thumb_dir, "thumbnail.jpg")
        thumbnail_path = None

        if _download_youtube_thumbnail(video_id, thumb_out):
...
def download_instagram_video(url: str, output_path: str) -> Dict:
    ...
    # ── INSTAGRAM — meta_client (yt-dlp, no instaloader 401s) ────────────────
    try:
        logger.info(f"⬇️ Downloading Instagram video via meta_client: {url}")
        ig_result = meta_client.download_video(url, output_path)

        if not ig_result.get("success"):
            logger.error(f"❌ Instagram meta_client download failed: {ig_result.get('error')}")
            return {"success": False, "metadata": {}, "post": None, "thumbnail_path": None}
        ...
        logger.info(
            f"✅ Instagram downloaded: @{meta.get('username')} | "
            f"thumb={'✅' if thumbnail_path else '❌'}"
        )
