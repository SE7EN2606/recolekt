...
- A block of consecutive ingredient-like lines.
...
LATIN_UNIT_TOKENS = [
    ...
    "tsp", "tbsp",
    "cup", "cups",
    r"c\.?\s*a\.?\s*s",  # FR abbreviations frequently used
    r"c\.?\s*a\.?\s*c",
...
# (No trailing \b?; no empty alternation)
...
def _normalize_caption(text: str) -> str:
    """
    Handle captions that might be stored with literal '\\n' sequences.
    """
    s = text or ""
    if "\\n" in s and "\n" not in s:
        s = s.replace("\\n", "\n")
...
def _extract_leading_qty(line: str) -> Optional[str]:
    """
    Extract a leading quantity token from a line, after removing bullets.
    """
...
def _line_is_qty_like(line: str) -> Tuple[bool, bool]:
    """
    Returns (is_qty_like, is_strong_unit_line)

    - qty-like if:
      A) starts with qty, or
      B) contains qty+unit anywhere (including fused, e.g., 20마리 / 200ml)
    - strong if unit is present (UNIT_ANY_RE) or fused qty+unit found
    """
    raw = line or ""
    ...
    # A) quantity-first (EU/US ingredient line style)
    lead = _extract_leading_qty(s)
    if lead:
        return (True, bool(UNIT_ANY_RE.search(s)))

    # B) fused qty+unit (Korean + attached Latin units)
    if FUSED_QTY_RE.search(s) and len(s) <= 90:
        return (True, True)

    # C) name-first with separated number and unit somewhere on the line
    # Example: "새우 20마리", "감자전분 8큰술", "물 200 ml"
...
def classify_reel_content(transcript: str, caption: str) -> Dict:
    ...
    if is_recipe:
        return {
            "label": "recipe",
            "score": 900,
            "reason": "FORCED:caption_qty_structure",
            "signals": {
                **f,
