...
from fetcher_api.api.auth_routes import get_user_id_from_request
        ...
        if user_id:
            ...
            if subscription_id:
                ...
                execute(
                    ...
                    DO UPDATE SET status=EXCLUDED.status, trial_ends_at=EXCLUDED.trial_ends_at,
                                  current_period_end=EXCLUDED.current_period_end, cancel_at_period_end=EXCLUDED.cancel_at_period_end, updated_at=now();
                    """,
                    (user_id, sub.get("id"), sub.get("status"), _ts(sub.get("trial_end")),
                     _ts(sub.get("current_period_end")), bool(sub.get("cancel_at_period_end", False))),
                )
        ...
        if user_id:
            execute(
                ...
                DO UPDATE SET status=EXCLUDED.status, trial_ends_at=EXCLUDED.trial_ends_at,
                              current_period_end=EXCLUDED.current_period_end, cancel_at_period_end=EXCLUDED.cancel_at_period_end, updated_at=now();
                """,
                (user_id, subscription_id, obj.get("status"), _ts(obj.get("trial_end")),
                 _ts(obj.get("current_period_end")), bool(obj.get("cancel_at_period_end", False))),
            )
